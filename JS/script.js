...
async function fetchPrayerTimes() {
  try {
    const response = await fetch(
      "https://api.aladhan.com/v1/timingsByCity?city=Mogadishu&country=Somalia&method=2"
    );
...
function updateNextPrayer(prayerTimes) {
  ...
  if (!nextPrayerTimeInMinutes) {
    ...
    const [hours, minutes] = prayerTimes.Fajar.replace(/AM|PM/, "")
      .split(":")
      .map(Number);
  ...
  document.querySelector(".next-prayer-time").textContent = `${String(
    hoursLeft
  ...
  // Update loading bar
  const totalTimeBetweenPrayers =
    nextPrayerTimeInMinutes - currentTimeInMinutes;
